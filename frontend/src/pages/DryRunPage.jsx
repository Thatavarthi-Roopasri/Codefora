...
export function DryRunPage() {
  ...
  const returnView = location.state?.returnView || "list";
  const returnProblemId = location.state?.selectedProblemId ?? (Number.isNaN(Number(id)) ? id : Number(id));

  function goBackToProblem() {
    if (returnView === "detail") {
      navigate('/problems', {
        state: { selectedProblemId: returnProblemId }
      });
      return;
    }
